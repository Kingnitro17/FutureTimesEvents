...
test('browser cannot mutate financial truth tables',()=>{assert.match(migration,/REVOKE INSERT, UPDATE, DELETE ON public\.orders/);assert.doesNotMatch(migration,/CREATE POLICY .* FOR (INSERT|UPDATE|DELETE).*financial_ledger/i)});
...
test('paid fulfillment is locked and uniqueness-backed',()=>{assert.match(migration,/FROM public\.orders WHERE id=p_order_id FOR UPDATE/);assert.match(migration,/tickets_order_fulfillment_unique/);assert.match(migration,/already_fulfilled/)});
