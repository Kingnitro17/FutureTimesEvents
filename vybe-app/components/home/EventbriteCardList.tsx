...
export function EventbriteCardList({ event }: { event: Event }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-row items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-white/20 transition-all duration-300"
    >
      {/* Thumbnail */}
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        <h3 className="text-sm font-bold text-[var(--text)] leading-tight mb-1.5 group-hover:text-blue-500 transition-colors line-clamp-2 capitalize">
          {event.title.toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}
        </h3>
        <div className="flex flex-col gap-1">
          <span className="flex items-center gap-1.5 text-[11px] text-[var(--text-muted)] font-medium truncate">
