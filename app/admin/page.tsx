...
const ADMIN_USERS = [
  ...
  { id: 'u2', name: 'Kim Lee', email: 'kim@email.com', role: 'user', status: 'active', spent: 890, color: '#F472B6' },
  { id: 'u3', name: 'Marcus Rivera', email: 'marcus@email.com', role: 'organizer', status: 'active', spent: 2400, color: '#4ADE80' },
  { id: 'u4', name: 'Taylor Swift', email: 'taylor@email.com', role: 'user', status: 'suspended', spent: 0, color: '#FB923C' },
  { id: 'u5', name: 'Dana Park', email: 'dana@email.com', role: 'user', status: 'active', spent: 560, color: '#60A5FA' },
  { id: 'u6', name: 'Jordan Wu', email: 'jordan@email.com', role: 'admin', status: 'active', spent: 3200, color: '#FACC15' },
];
            ...
            {MOCK_EVENTS.map(ev => (
              <div key={ev.id} className="glass rounded-xl p-4 border border-[var(--border)] flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-[var(--text)]">{ev.title}</p>
                  <p className="text-xs text-[var(--text-muted)]">{ev.date} · {ev.venue} · {ev.attendees} attendees</p>
                </div>
