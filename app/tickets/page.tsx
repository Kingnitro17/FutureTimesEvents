      ...
      <div className="border-b border-[var(--border)]" style={{ background: 'var(--bg)' }}>
        <div className="container py-6 sm:py-10">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' }}>
            ...
            <p className="text-sm text-[var(--text-muted)]">{tickets.length} tickets in your wallet</p>
          </motion.div>
        </div>
      </div>
            ...
            {(['all','upcoming','past'] as const).map(f => (
              <motion.button key={f} onClick={() => setFilter(f)}
                ...
                className={`btn btn-sm capitalize transition-all ${
                  filter === f ? 'btn-grad text-white' : 'btn-ghost'
                }`}>
                {f}
              </motion.button>
            ))}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-4">
            {filtered.map((ticket, i) => {
              const s = STATUS[ticket.status as keyof typeof STATUS] || STATUS.past;
              ...
              return (
                ...
                >
                  ...
                  {/* QR block — left sidebar on ALL screen sizes */}
                  <div className="flex flex-col items-center justify-center p-3 border-r border-[var(--border)] bg-white dark:bg-black/10 shrink-0 w-[90px] sm:w-[120px]">
                    ...
                    <div className="p-5">
                        ...
                        {isOpen && (
                          ...
                          >
                            <div className="border-t border-[var(--border)] pt-3 mb-4 space-y-1.5 text-xs text-[var(--text-muted)]">
                              ...
                              {ticket.tableBooking && <div className="flex justify-between"><span>Table</span><span className="text-[var(--text)] font-medium">{ticket.tableBooking.name}</span></div>}
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Link href={`/events/${ticket.eventId}`} className="btn btn-sm btn-ghost gap-1.5">👁️ Event</Link>
                      </div>
                    </div>
                  </div>
                </motion.div>
